...
function updateDecorations() {
	...
	try {
		...
		// 查找折叠的数组
		const foldingRanges = editor.document.getText().split('\n');
		for (let i = 0; i < foldingRanges.length; i++) {
			const line = foldingRanges[i];
			if (line.includes('[') && !line.includes(']')) {
				try {
					const arrayContent = getArrayContent(text, editor.document.offsetAt(new vscode.Position(i, 0)));
					if (arrayContent) {
